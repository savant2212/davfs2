...
typedef struct {
    dav_xattrs *results;         /* Start of the linked list of dav_xattrs. */
} propfind_xattr_context;
...
int propset_iterator(void* userdata, const ne_propname* name, const char* value, const ne_status* status);
...
int propset_iterator(void* userdata, const ne_propname* name, const char* value, const ne_status* status){
	propfind_xattr_context* ct = (propfind_xattr_context*)userdata;

	dav_xattrs* list = ct->results;
	dav_xattrs* last;
	last = list;

	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "iterate %s:%s", name->name, value);
	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "ct.results address %x", list);

	if ( list == NULL ){
		list = ne_malloc(sizeof(dav_xattrs));
		if( userdata == NULL ){
			return ENOMEM;
		}
		ct->results=list;
		...
		last->next = (dav_xattrs*)ne_malloc(sizeof(dav_xattrs));
...
prop_result(void *userdata, const ne_uri *uri, const ne_prop_result_set *set)
{
    ...
    *to = 0;
    ...
    propfind_xattr_context ct;
    ct.results = NULL;
    syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "getting all attributes to %s", result->path);
    ne_propset_iterate(set, propset_iterator, &ct );
    result->xattrs = ct.results;

    syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "result->xattr %x", result->xattrs);
