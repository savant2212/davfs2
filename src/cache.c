...
xml_start_string(void *userdata, int parent, const char *nspace,
                 const char *name, const char **atts);
static int
xml_start_xattr(void *userdata, int parent, const char *nspace,
              const char *name, const char **atts);
static int
xml_end_xattr(void *userdata, int state, const char *nspace, const char *name);

static int
xml_start_xattrs(void *userdata, int parent, const char *nspace,
              const char *name, const char **atts);
static int
xml_end_xattrs(void *userdata, int state, const char *nspace, const char *name);
/* Auxiliary. */
		...
		if( strncmp(name,tmp_str, strlen(name)) == 0){
	    	...
	    	if( list-> value == NULL){
	    		...
	    		*size=strlen(list->value);
	    		syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "getxattr %s:%s len:", tmp_str, list->value, *size);
...
{
	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "dav_listxattr called");
	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "node path: %s", node->path);
    if (!is_valid(node)){
    	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "node param isn't valid");
        return ENOENT;
    }
    ...
    syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "list xattrs for %s. count %i, &xattrs %p",node->path, node->xattr_count, node->xattrs);
    syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "buf size: %u", size);
    dav_xattr_item* list = node->xattrs;

    if (*size == 0) {
    	*size = node->xattr_count * 255; //254 - max len of xattr name + byte for line end
    	// may be realloc buffer?
    }
    char* ptr = buf;
    int len=0;
    while(list != NULL ){
    	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "attr: %s, next: %x",list->name, list->next);
...
add_node(dav_node *parent, dav_props *props)
{
    ...
    syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "copy xattrs for %s", props->path );
    dav_xattrs *lst = props->xattrs;
    dav_xattrs *to_free;
    dav_xattr_item* item = node->xattrs;
    syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "props->xattrs address %p", props->xattrs );

    while( lst != NULL ){
    	if( item == NULL ){
    		item = (dav_xattr_item*)ne_malloc(sizeof(dav_xattr_item));
    		item->next = NULL;
    		node->xattrs = item;
    	} else {
    		item->next = (dav_xattr_item*)ne_malloc(sizeof(dav_xattr_item));
    		item = item->next;
    		item->next = NULL;
    	}
    	syslog(LOG_MAKEPRI(LOG_DAEMON, LOG_DEBUG), "copy xattr %s:%s", lst->name, lst->value );
    	item->name = lst->name;
    	if( lst-> value != NULL ){
    		item->value = strdup(lst->value);
    	} else {
    		item->value = NULL;
    	}
...
parse_index(void)
{
    ...
    ne_xml_push_handler(parser, xml_start_xattrs, NULL, xml_end_xattrs, &node);
    ne_xml_push_handler(parser, xml_start_xattr, NULL, xml_end_xattr, &node);
...
write_node(dav_node *node, FILE *file, const char *indent)
{
    ...
    dav_xattr_item *xattr = node->xattrs;
    if (fprintf(file, "%s<d:%s>\n", ind, type[XATTRS]) < 0)
    	return -1;

    char *ind_xa1 = ne_concat(ind, "  ", NULL);
    char *ind_xa2 = ne_concat(ind_xa1, "  ", NULL);
	while (xattr != NULL) {
		if (fprintf(file, "%s<d:%s>\n", ind_xa1, type[XATTR]))
		            return -1;

		if (fprintf(file, "%s<d:%s><![CDATA[%s]]</d:%s>\n", ind_xa2, type[XATTR_NAME], xattr->name, type[XATTR_NAME]))
				            return -1;
		if (fprintf(file, "%s<d:%s><![CDATA[%s]]</d:%s>\n", ind_xa2, type[XATTR_VALUE], xattr->value, type[XATTR_VALUE]))
						            return -1;
...
xml_end_string(void *userdata, int state, const char *nspace, const char *name)
{
    ...
    dav_xattr_item* lst = (*((dav_node **) userdata))->xattrs;
                 ...
                 const char *name, const char **atts)
{
    ...
    if (strcmp(name, type[PATH]) == 0) {
        ...
        ret = MIME;
    } else if (strcmp(name, type[XATTR_NAME]) == 0) {
            ret = XATTR_NAME;
    } else if (strcmp(name, type[XATTR_VALUE]) == 0) {
            ret = XATTR_VALUE;
    } else {
        return 0;
    }

    if (parent != DDIR && parent != REG && parent != XATTR)
        return -1;
...
xml_start_xattrs(void *userdata, int parent, const char *nspace,
               const char *name, const char **atts)
{
    if (strcmp(name, type[XATTRS]) != 0) {
        return 0;
    }

    if (parent != REG && parent != DDIR )
        return -1;
...
xml_end_xattr(void *userdata, int state, const char *nspace, const char *name)
{
	dav_node* node = (*((dav_node **) userdata));
	dav_xattr_item* lst = node->xattrs;
	if(node->xattr_count == 0 ){
		lst = (dav_xattr_item*)ne_malloc(sizeof(dav_xattr_item));
		...
		lst->next = (dav_xattr_item*)ne_malloc(sizeof(dav_xattr_item));
